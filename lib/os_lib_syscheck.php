...
function __os_getchanges($file, &$g_last_changes, $_name)
{
    ...
    while(!feof($fp))
    {
	    ...
	    /*if(sizeof($g_last_changes['files']) < 100)*/
         /*   if (isset($g_last_changes['files']) && is_array($g_last_changes['files']) && count($g_last_changes['files']) <100)
            {
...
if (!isset($g_last_changes['files']) || !is_array($g_last_changes['files'])) {
    $g_last_changes['files'] = [];
}

if (count($g_last_changes['files']) < 100) {
    ...
    rsort($g_last_changes['files']);
} else if (isset($g_last_changes['lowest']) && $time_stamp > $g_last_changes['lowest']) {
    rsort($g_last_changes['files']);
    array_pop($g_last_changes['files']);
    $g_last_changes['files'][] = array($time_stamp, $_name, $sk_file_name);
}
